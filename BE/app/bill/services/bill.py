...
from core.common.exceptions import (
    BillFasterBadRequestException, BillFasterNotFoundException,
)
...
class BillService(BillFasterBaseService):
    ...
    async def _create_or_update_bill_items(self, bill_id, bill_items: list[BillItemCrUpSchema]):
    ...
    async def _create_or_update_bill_participants(self, bill_id, bill_participants: list):
    ...
    async def create(self, create_schema: BillCrUpSchema):
        try:
            data_fields = create_schema.model_dump(exclude={"bill_items", "bill_participants"})
            data_fields["bill_number"] = f"BF{datetime.datetime.now().strftime('%Y%m%d%H%M%S')}{str(uuid.uuid4())}".replace("-", "")
            new_bill = Bill(**data_fields)
            self.db_session.add(new_bill)
            await self.db_session.flush()
            await self._create_or_update_bill_items(new_bill.id, create_schema.bill_items)
            await self._create_or_update_bill_participants(new_bill.id, create_schema.bill_participants)
    ...
    async def update(self, bill_number: str, update_schema: BillCrUpSchema):
        bill = await self._get_bill_by_bill_number(bill_number)
        if not bill:
            raise BillFasterNotFoundException()
        try:
            data_fields = update_schema.model_dump(exclude={"bill_items", "bill_participants", "bill_number"})
            for key, value in data_fields.items():
                setattr(bill, key, value)

            await self._create_or_update_bill_items(bill.id, update_schema.bill_items)
            await self._create_or_update_bill_participants(bill.id, update_schema.bill_participants)
            await self.db_session.commit()
            return {
    ...
    async def get_by_bill_number(self, bill_number: str) -> dict:
        try:
            bill = await self._get_bill_by_bill_number(bill_number)
            if not bill:
                raise BillFasterNotFoundException()
